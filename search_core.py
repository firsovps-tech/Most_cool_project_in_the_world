...
class NeuralHybridProductSearch:
    """
    Этот файл НЕ подбирает итоговые веса руками.

    Он только считает признаки для бустинга:
    exact_id_score, bm25_score, field_embedding_score,
    price_score, отдельные embedding_score по полям и product_has_*.
    """
    ...
    def get_candidate_indexes(
        ...
        for index in np.argsort(exact_id_scores)[::-1][:id_limit]:
            index = int(index)

            if exact_id_scores[index] > 0:
                candidate_indexes.add(index)

        for index in np.argsort(bm25_scores)[::-1][:bm25_limit]:
            candidate_indexes.add(int(index))

        for index in np.argsort(embedding_scores)[::-1][:embedding_limit]:
            candidate_indexes.add(int(index))
