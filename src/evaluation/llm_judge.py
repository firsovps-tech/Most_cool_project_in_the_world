...
CACHE_FILE = Path("data") / "judge_cache.json"
...
""".strip()
    ...
    for i in range(retries):
        try:
            ...
            print(f"  [Ошибка API/JSON, попытка {i + 1}/{retries}]: {error}", flush=True)
            if i < retries - 1:
                time.sleep(5)
                continue
    ...
    if article_to_id:
        ...
        if query_clean in article_to_id:
            ...
            for word in query_clean.split():
                word = normalize_article(word)
                if word in article_to_id:
                    if is_common_word_article(word):
                        continue
                    matched_art = word
                    break
        ...
        if matched_art:
            ...
            save_cache(cache)
            print(f"  [АРТИКУЛ BYPASS] {product_id} | Оценка: {score}", flush=True)
            return score, reason

    print(f"  Запрос к LLM для {product_id}...", flush=True)
